...
describe("on match", () => {
    ...
    it("should create a match", (done) => {
        ...
        const client = createClient(); //reset data
        const client2 = createClient2();
        const client3 = createClient2();
        ...
        client3.on("_data", res => {
            ...
            assert.equal(res.matches[0].player1.ime, req2.ime, "Must be equal to first players name");
            assert(res.matches[0].player1.playing, "On match start must equal true");
            assert.equal(res.matches[0].player1.points, 0, "Must be equal to 0 at start of game");

            assert.equal(res.matches[0].player2.ime, req.ime, "Must be equal to second players name");
            assert(res.matches[0].player2.playing, "On match start must equal true");
            assert.equal(res.matches[0].player2.points, 0, "Must be equal to 0 at start of game");

            assert.equal(res.players[client.id].ime, req.ime, "On signin must equal to player1 ime")
            assert(res.players[client.id].playing, "On match start must equal true")
            assert.equal(res.players[client.id].points, 0, "Must be equal to 0 at start of match");
            ...
            assert.equal(res.matches.length, 1, "Must equal to 1 after creating a match");
    ...
    it("should create two matches", (done) => {
        ...
        const client1 = createClient(); //reset data
        const client2 = createClient2();
        const client3 = createClient2();
        ...
        client2.on("_data", (res) => {
            ...
            assert.equal(res.matches.length, 1, "Must equal to 1 after first match started");
        ...
        client3.on("signin", res => {
            ...
            //try to create a match & possible testing of "No players available for a game" err, before fourth player signin ?
        });
